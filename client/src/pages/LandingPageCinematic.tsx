...
import BootSequenceEnhanced from '../components/BootSequenceEnhanced';
import TelemetryPanelEnhanced from '../components/TelemetryPanelEnhanced';
import NeuralActivityEnhanced from '../components/NeuralActivityEnhanced';
import QuantumShieldEnhanced from '../components/QuantumShieldEnhanced';
import HeroDragonContainer from '../components/HeroDragonContainer';
...
const STAGE_INFO = [
  ...
  { title: 'TACTICAL INTEL ONLINE', subtitle: 'Acquiring global threat telemetry and bringing tactical panels online.' },
  { title: 'SYSTEM ONLINE', subtitle: 'Primary command interface ready. Awaiting operator activation.' },
];

const LandingPageCinematic: React.FC<LandingPageProps> = ({ onEnter }) => {
  ...
  // Mouse parallax for cinematic depth
  useEffect(() => {
  ...
  // Synchronize boot lines with progress
  useEffect(() => {
    if (stage !== 2) return;
  ...
  // Update status messages based on stage
  useEffect(() => {
    if (stage === 1) {
      setTimeout(() => {
  ...
  // Handle attack telemetry from map
  ...
  return (
    <div
      ref={pageRef}
      className="relative w-screen h-screen overflow-hidden bg-[#0b000f] text-white flex flex-col justify-between select-none font-mono"
      style={{
        perspective: 1400,
    ...
    >
      ...
      <CyberParticles intensity={0.6} />

      {/* Cinematic Background */}
      <CinematicBackground stage={stage} />

      {/* Matrix Code Rain (Stage 2) */}
      {stage === 2 && <MatrixStream opacity={0.2} />}

      {/* Attack Map Background (Stage 3+) */}
      {stage >= 3 && (
      ...
      <div className="absolute inset-0 z-10 pointer-events-none flex items-center justify-center">
        {stage >= 1 && (
          <HeroDragonContainer stage={stage} mouseOffset={mouseOffset} />
        )}
      </div>
      ...
      <header
        ...
        style={{
          transform: `translate3d(calc(${mouseOffset.x}px / 2), calc(${mouseOffset.y}px / 2), 0)`,
        }}
      >
      ...
      <div
        ...
        ref={stageBadgeRef}
        style={{
          transform: `translate3d(calc(${mouseOffset.x}px / 1.6), calc(${mouseOffset.y}px / 1.6), 0)`,
        }}
      >
        <motion.div
        ...
        >
          <span className="text-[10px] uppercase tracking-[0.35em] text-red-400">
            {STAGE_INFO[stage - 1].title}
          </span>
          <span className="text-[9px] text-gray-400 max-w-[18rem] text-center">
            {STAGE_INFO[stage - 1].subtitle}
          </span>
        </motion.div>
      </div>
      ...
      {/* ==================== MAIN HUD SECTION ==================== */}
      <main
        className="relative flex-grow flex flex-col lg:flex-row items-start justify-between gap-6 py-6 z-20 pointer-events-none px-6"
        style={{
          transform: `translate3d(calc(${mouseOffset.x}px / 2.2), calc(${mouseOffset.y}px / 2.2), 0)`,
        }}
      >
        {/* LEFT PANEL: Boot Sequence & Neural Activity */}
        <div className="flex flex-col gap-4 w-full lg:w-[28rem] pointer-events-auto">
          <BootSequenceEnhanced
            bootLines={bootLines}
            percent={percent}
          ...
          {stage >= 3 && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <NeuralActivityEnhanced values={audioOscillation} height={140} />
              <QuantumShieldEnhanced height={140} />
            </div>
          )}
        </div>

        {/* CENTER: Dragon Hero & Welcome Message */}
        <div className="relative w-full lg:w-[42rem] pointer-events-none flex flex-col items-center justify-center">
          {stage === 4 && (
            <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-20 flex flex-col items-center justify-center text-center px-6">
              <motion.h2
                ...
                style={{
                  textShadow:
                    '0 0 35px rgba(255,0,60,0.6), 0 0 70px rgba(255,0,60,0.2)',
                }}
              >
                WELCOME COMMANDER
              </motion.h2>
          ...
          {stage >= 3 && (
            <TelemetryPanelEnhanced entries={telemetryEntries} maxEntries={8} />
          )}
        </div>
      </main>

      {/* ==================== FOOTER SECTION ==================== */}
      <footer className="relative w-full z-40 flex flex-col items-center gap-4 px-6">
        ...
        <AnimatePresence>
          {stage === 4 && (
            <motion.div
              ...
              className="flex flex-col sm:flex-row gap-4 items-center justify-center w-full max-w-2xl pointer-events-auto"
            >
              <div className="relative flex items-center justify-center">
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  className="relative z-10 w-full sm:w-auto px-10 py-3 bg-red-500/15 border border-red-400/60 text-red-100 font-mono text-sm tracking-[0.25em] rounded-full shadow-[0_0_30px_rgba(255,0,60,0.24)] hover:border-red-100 hover:bg-red-500/25 transition-all duration-300 overflow-hidden uppercase font-bold"
                >
                ...
                <span className="absolute inset-x-0 -bottom-5 text-[9px] text-gray-400 uppercase tracking-[0.35em]">
                  {isActivating
                    ? `ENGAGING CORE ${activationProgress}%`
                    : 'FINAL CORE ENGAGEMENT READY'}
                </span>
              </div>
            </motion.div>
          )}
        ...
        <div className="w-full border-t border-red-500/25 bg-[#0b000f]/80 backdrop-blur-md px-6 py-3 flex flex-col md:flex-row items-center justify-between text-[9px] text-gray-500 gap-2 mt-4 rounded-lg">
          <div className="flex items-center gap-3">
            <span className="text-[10px] font-bold text-red-500 tracking-[0.1em] uppercase animate-pulse">
              NEURAL FEED
            </span>
            <span
              className="text-gray-300 tracking-wider font-semibold border-l border-red-950 pl-3 transition-colors"
              style={{ color: activeLogColor }}
            >
              {activeLogText}
            </span>
          </div>
          <div className="flex gap-4">
            ...
            <span className="tracking-widest">© 2026 DANGEN AI CYBER DEFENSE PLATFORM</span>
          </div>
        </div>
      </footer>
      ...
      {isActivating && (
        <div
          ref={activationOverlayRef}
          className="absolute inset-0 z-50 pointer-events-none bg-[#ff0016]/10 backdrop-blur-sm"
        >
...
export default LandingPageCinematic;
